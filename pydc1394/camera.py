...
class DC1394Library(object):
	...
	cameralist:		a list of dicts. Each camera has a dict, containing:
		...
		get_list():	reinitializes the camera list
	"""
	def __init__( self ):
...
class CameraProperty(object):
	...
	def pos_modes(self):
		...
		return [ feature_mode_vals[modes.modes[i]] for i in range(modes.num) ]
	...
	def mode():
		...
		def fset(self, value):
			key, = [ k for k,v in feature_mode_vals.items() if v == value ]
